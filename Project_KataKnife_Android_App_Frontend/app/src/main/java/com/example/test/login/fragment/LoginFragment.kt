...
class LoginFragment(private val updatePage: UpdatePage) : Fragment() {
    ...
    private fun login() {
        ...
        when (LoginUtil(requireContext()).userLogin(userId, password)) {
            ...
            //login fail
            LoginUtil.LOGIN_CONNECTION_FAIL -> toast(getString(R.string.connection_fail))
            LoginUtil.INVALID_LOGIN_INFO -> toast(getString(R.string.login_wrong_id))
        }
    }
